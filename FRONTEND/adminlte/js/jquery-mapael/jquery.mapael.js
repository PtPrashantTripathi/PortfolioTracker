    ...
    Mapael.prototype = {
        ...
        init: function () {
            ...
            $.each(self.mapConf.elems, function (id) {
                ...
                // draw area
                self.areas[id].mapElem = self.paper.path(
                    self.mapConf.elems[id],
                );
            });

            // Hook that allows to add custom processing on the map
            if (self.options.map.beforeInit)
                self.options.map.beforeInit(
                    self.$container,
                    self.paper,
                    self.options,
                );
            ...
            self.$container.on(
                ...
                function (event) {
                    ...
                    mapMouseMoveTimeoutID = setTimeout(function () {
                        ...
                        if (dataTypeToElementMapping[type] !== undefined) {
                            self.elemHover(
                                dataTypeToElementMapping[type][id],
                                event,
                            );
                        } else if (
                            type === "legend-elem" ||
            ...
            $.each(self.customEventHandlers, function (eventName) {
                ...
                self.$container
                    ...
                    .on(fullEventName, "[data-id]", function (e) {
                        ...
                        if (
                            !self.panning &&
                            self.customEventHandlers[eventName][type] !==
                                undefined &&
                            self.customEventHandlers[eventName][type][id] !==
                                undefined
                        ) {
        ...
        initElem: function (id, type, elem) {
            ...
            if (elem.options.text && elem.options.text.content !== undefined) {
                ...
                elem.options.text.attrs["text-anchor"] =
                ...
                .on(
                    ...
                    function (e) {
                        ...
                        if (mousedown && currentLevel !== 0) {
                            var offsetX =
                                ((previousX - pageX) /
                                    (1 + currentLevel * zoomOptions.step)) *
        ...
        mapPagePositionToXY: function (pageX, pageY) {
            ...
            return {
                x:
                    zoomFactor * initFactor * (pageX - offset.left) +
                    self.zoomData.panX,
                y:
                    zoomFactor * initFactor * (pageY - offset.top) +
                    self.zoomData.panY,
            };
        },
        ...
        onZoomEvent: function (e, zoomOptions) {
            ...
            if (zoomOptions.area !== undefined) {
                ...
                var areaMargin =
                    zoomOptions.areaMargin !== undefined
                        ? zoomOptions.areaMargin
                        : 10;
                ...
                if (zoomOptions.level !== undefined) {
                    if (typeof zoomOptions.level === "string") {
                        ...
                        if (
                            ...
                            zoomLevel =
                                self.zoomData.zoomLevel +
                                parseInt(zoomOptions.level, 10);
                        } else {
                        ...
                        if (zoomOptions.level < 0) {
                            // zoomLevel is relative
                            zoomLevel =
                                self.zoomData.zoomLevel + zoomOptions.level;
                        } else {
                            // zoomLevel is absolute
                ...
                if (zoomOptions.plot !== undefined) {
                    ...
                    if (zoomOptions.x === undefined) {
                        zoomOptions.x =
                            self.currentViewBox.x + self.currentViewBox.w / 2;
                    }

                    if (zoomOptions.y === undefined) {
                        zoomOptions.y =
                            self.currentViewBox.y + self.currentViewBox.h / 2;
                    }
                }
            }
            ...
            if (zoomLevel === 0) {
                ...
                panX = Math.min(
                    Math.max(0, panX),
                    self.mapConf.width - panWidth,
                );
                panY = Math.min(
                    Math.max(0, panY),
                    self.mapConf.height - panHeight,
                );
            }
        ...
        showElemByRange: function (ranges, elems, hiddenOpacity, animDuration) {
            ...
            $.each(ranges, function (valueIndex) {
                ...
                $.each(elems, function (id) {
                    ...
                    if (
                        (range.min !== undefined &&
                            elemValue[valueIndex] < range.min) ||
                        (range.max !== undefined &&
                            elemValue[valueIndex] > range.max)
                    ) {
        ...
        onUpdateEvent: function (e, opt) {
            ...
            $.each(self.links, function (id) {
                ...
                if (
                    (typeof opt.mapOptions === "object" &&
                        ((typeof opt.mapOptions.map === "object" &&
                            typeof opt.mapOptions.map.defaultLink ===
                                "object") ||
                            (typeof opt.mapOptions.links === "object" &&
                                typeof opt.mapOptions.links[id] ===
                                    "object"))) ||
                    opt.replaceOptions === true
                ) {
            ...
            if (typeof opt.setLegendElemsState === "object") {
                ...
                $.each(
                    opt.setLegendElemsState,
                    function (legendCSSClass, action) {
                        // Search for the legend
                        var $legend = self.$container.find(
                            "." + legendCSSClass,
                        )[0];
                        if ($legend !== undefined) {
                            ...
                            $("[data-type='legend-elem']", $legend).each(
                                function (id, elem) {
                                    if (
                                        ($(elem).attr("data-hidden") === "0" &&
                                            action === "hide") ||
                                        ($(elem).attr("data-hidden") === "1" &&
                                            action === "show")
                                    ) {
                                        // Toggle state of element by clicking
                                        $(elem).trigger("click", {
                ...
                // Default : "show"
                var action =
                    opt.setLegendElemsState === "hide" ? "hide" : "show";
        ...
        setPlotCoords: function (plot) {
            ...
            if (plot.options.x !== undefined && plot.options.y !== undefined) {
                ...
                var areaBBox =
                    self.areas[plot.options.plotsOn].mapElem.getBBox();
                plot.coords = {
                    x: areaBBox.cx,
                    y: areaBBox.cy,
                };
        ...
        drawLinksCollection: function (linksCollection) {
            ...
            $.each(linksCollection, function (id) {
                ...
                if (
                    ...
                    p1.latitude !== undefined &&
                    p1.longitude !== undefined
                ) {
                    coordsP1 = self.mapConf.getCoords(
                        p1.latitude,
                        p1.longitude,
                    );
                } else {
                ...
                if (
                    ...
                    p2.latitude !== undefined &&
                    p2.longitude !== undefined
                ) {
                    coordsP2 = self.mapConf.getCoords(
                        p2.latitude,
                        p2.longitude,
                    );
                } else {
        ...
        updateElem: function (elem, animDuration) {
            ...
            if (elem.textElem) {
                ...
                if (
                    elem.options.size ||
                    (elem.options.width && elem.options.height)
                ) {
                    if (
                        elem.options.type === "image" ||
                        elem.options.type === "svg"
                    ) {
                        plotOffsetX =
                            (elem.options.width - mapElemBBox.width) / 2;
                        plotOffsetY =
                            (elem.options.height - mapElemBBox.height) / 2;
                    } else {
                        plotOffsetX =
                            (elem.options.size - mapElemBBox.width) / 2;
                        plotOffsetY =
                            (elem.options.size - mapElemBBox.height) / 2;
                    }
        ...
        drawLegend: function (
            ...
            for (i = 0; i < legendOptions.slices.length; ++i) {
                ...
                if (
                    ...
                    yCenterCurrent +=
                        (scale * sliceOptions[i].attrs.height) / 2;
                }
                ...
                if (
                    ...
                    if (legendType === "area") {
                        if (legendOptions.mode === "horizontal") {
                            ...
                            y =
                                yCenter -
                                0.5 * scale * sliceOptions[i].attrs.height;
                        } else {
                        ...
                        if (sliceOptions[i].type === "image") {
                            ...
                            sliceOptions[i].attrs.transform =
                                "m" +
                                (scale * sliceOptions[i].width) /
                                    legendElemBBox.width +
                                ",0,0," +
                                (scale * sliceOptions[i].height) /
                                    legendElemBBox.height +
                                "," +
                                x +
                                "," +
                                y +
        ...
        handleClickOnLegendElem: function (
            ...
            $.each(mapElems, function (y) {
                ...
                if (
                    self.getLegendSlice(elemValue, legendOptions) ===
                    sliceOptions
                ) {
                    if (hidden === "0") {
            ...
            if (
                (opts.hideOtherElems === undefined ||
                    opts.hideOtherElems === true) &&
        ...
        createLegends: function (legendType, elems, scale) {
            ...
            for (var j = 0; j < legendsOptions.length; ++j) {
                if (
                    ...
                    $("." + legendsOptions[j].cssClass, self.$container)
                        .length !== 0
                ) {
        ...
        elemHover: function (elem, event) {
            ...
            if (elem.options.tooltip !== undefined) {
                ...
                if (typeof elem.options.tooltip.offset === "object") {
                    if (
                        typeof elem.options.tooltip.offset.left !== "undefined"
                    ) {
                        offsetLeft = elem.options.tooltip.offset.left;
                    }
                    if (
                        typeof elem.options.tooltip.offset.top !== "undefined"
                    ) {
                ...
                if (typeof elem.options.tooltip.overflow === "object") {
                    if (elem.options.tooltip.overflow.right === true) {
                        tooltipPosition.left =
                            mouseX - self.$map.offset().left + 10;
                    }
                    if (elem.options.tooltip.overflow.bottom === true) {
                        tooltipPosition.top =
                            mouseY - self.$map.offset().top + 20;
                    }
                }
        ...
        animateViewBox: function (
            ...
            var easingFormula =
                Raphael.easing_formulas[easingFunction || "linear"];
            ...
            var computeNextStep = function () {
                ...
                if (elapsed < durationWithMargin) {
                    ...
                    if (
                        ...
                        var realElapsed =
                            new Date().getTime() - oldZoomAnimStartTime;
                    ...
                    self.zoomAnimID =
                        self.requestAnimationFrame(computeNextStep);
                } else {
                    /* Zoom animation done ! */
