    ...
    function initNevigation(plot, options) {
        ...
        function onMouseWheel(e, delta) {
            var maxAbsoluteDeltaOnMac = 1,
                isMacScroll =
                    Math.abs(e.originalEvent.deltaY) <= maxAbsoluteDeltaOnMac,
        ...
        plot.zoom = function (args) {
            ...
            for (var key in axes) {
                ...
                if (
                    (!opts.axisZoom && args.axes) ||
                    (!args.axes && !opts.plotZoom)
                ) {
                    continue;
                }
        ...
        plot.pan = function (args) {
            ...
            $.each(args.axes || plot.getAxes(), function (_, axis) {
                ...
                if (
                    (!opts.axisPan && args.axes) ||
                    (!opts.plotPan && !args.axes)
                ) {
                    return;
                }
                ...
                if (d !== 0) {
                    var navigationOffsetBelow = saturated.saturate(
                            axis.c2p(axis.p2c(axis.min) + d) -
                                axis.c2p(axis.p2c(axis.min)),
                        ),
                        navigationOffsetAbove = saturated.saturate(
                            axis.c2p(axis.p2c(axis.max) + d) -
                                axis.c2p(axis.p2c(axis.max)),
                        );
        ...
        var lockDeltaDirection = function (delta) {
            if (
                ...
                lockedDirection =
                    Math.abs(delta.x) < Math.abs(delta.y) ? "y" : "x";
            }
        ...
        plot.smartPan = function (
            ...
            var snap = smartLock ? true : shouldSnap(delta),
                ...
                opts;
            delta = smartLock
                ? lockDeltaDirection(delta)
                : adjustDeltaToSnap(delta);
            ...
            if (!preventEvent) {
                plot.getPlaceholder().trigger("plotpan", [
                    plot,
                    delta,
                    panAxes,
                    initialState,
