...
function priceFormat(value, short=false) {
    ...
    if (short) {
        ...
        if (value >= 1e7)
            return `${sign}₹${(value / 1e7).toFixed(2)}Cr`;
        if (value >= 1e5)
            return `${sign}₹${(value / 1e5).toFixed(2)}L`;
        if (value >= 1e3)
...
function parseNum(value) {
    return parseFloat(value) === parseInt(value) ? parseInt(value) : parseFloat(parseFloat(value).toFixed(2));
}
...
function calcDays(date1, date2=new Date()) {
    ...
    const differenceInDays = Math.abs(Math.floor((secondDate - firstDate) / (1000 * 60 * 60 * 24)));
...
function loadCurrentHoldingsDataTable(data) {
    const headers = ["Stock Name", "Qty.", "Avg Price", "Invested Value", "CMP Price", "Current Value", "Unrealized PNL", "PNL Percentage", "Holding Days", ];

    const cellData = data.map( (record) => {
        const pnlClass = record.pnl_amount < 0 ? "text-danger" : "text-success";
        return [createCell(`${record.segment === "EQ" ? record.symbol : record.scrip_name} (${record.segment})`), createCell(parseNum(record.total_quantity)), createCell(priceFormat(record.avg_price)), createCell(priceFormat(record.total_amount)), createCell(priceFormat(record.close_price), [pnlClass]), createCell(priceFormat(record.close_amount), [pnlClass]), createCell(priceFormat(record.pnl_amount), [pnlClass]), createCell(`${record.pnl_amount >= 0 ? "+" : ""}${parseNum((record.pnl_amount * 100) / record.total_amount)}%`, [pnlClass]), createCell(calcDays(record.min_datetime)), ];
    }
    );
...
function loadDividendDataTable(stockWiseData) {
    ...
    const uniqueYears = Array.from(new Set(stockWiseData.flatMap(item => item.data.map(subItem => subItem.year))// Extract years from each sub-item
    )).sort( (a, b) => a - b);

    const headers = ["Stock Name", ...uniqueYears.map(year => `Year-${year}`), "Total", ];
    ...
    const cellData = stockWiseData.map(record => {
        ...
        uniqueYears.forEach(year => {
            const yearData = record.data.find(subRecord => subRecord.year === year);
            // Find data for the specific year
            result.push(yearData ? createCell(priceFormat(yearData.dividend_amount), ["text-success"]) : createCell('-', ["text-info"]));
        }
        );

        // Add the total dividend amount for the symbol
        result.push(createCell(priceFormat(record.dividend_amount), ["text-success"]));
        return result;
    }
    );
...
function loadProfitLossDataTable(data) {
    const headers = ["Stock Name", "Qty.", "Avg Price", "Sell Price", "Realized PNL", "PNL Percentage", "Holding Days", ];
    const cellData = data.map( (record) => {
        const pnlFlag = record.pnl < 0;
        return [createCell(`${record.symbol} (${record.segment})`), createCell(parseNum(record.quantity)), createCell(priceFormat(record.avg_price)), createCell(priceFormat(record.sell_price)), createCell(priceFormat(record.pnl), pnlFlag ? ["text-danger"] : ["text-success"]), createCell(`${pnlFlag ? "" : "+"}${parseNum((record.pnl * 100) / (record.avg_price * record.quantity))}%`, pnlFlag ? ["text-danger"] : ["text-success"]), createCell(record.days), ];
    }
    );
...
function renderSummary(elementId, summaryItems) {
    ...
    elem.innerHTML = summaryItems.map( (item) => `
...
function loadHoldingsTrandsChart(data) {
    ...
    const chart = new ApexCharts(document.getElementById("holdingsTrandsChart"),options);
    chart.render();
    return chart;
}
...
function updateDividendSummary(yearWiseData) {
    ...
    const totalDividendAmount = yearWiseData.reduce( (sum, item) => sum + item.dividend_amount, 0);
    document.getElementById("totalDividend").textContent = priceFormat(totalDividendAmount);
...
function loadDividendChart(yearWiseData) {
    ...
    // Extract years and dividend amounts for the chart
    const years = yearWiseData.map(item => item.year);
    ...
    const options = {
        ...
        series: [{
            ...
            // Series name
            data: dividendAmounts // Series data (dividend amounts)
        }],
        xaxis: {
            categories: years,
        ...
        yaxis: {
            title: {
                text: 'Dividend Amount (in currency)'// Y-axis title
            }
        },
        title: {
    ...
    // Render the chart
    const chart = new ApexCharts(document.getElementById("dividendBarChart"),options);
    chart.render();
    return chart;
}
...
async function main() {
    const apiPath = window.location.hostname === "ptprashanttripathi.github.io" ? "https://raw.githubusercontent.com/PtPrashantTripathi/PortfolioTracker/main/DATA/API/API_data.json" : "../DATA/API/API_data.json";
    ...
    updateFinancialSummary(apiData.financial_summary.invested_value, apiData.financial_summary.current_value, apiData.financial_summary.pnl_value);
    updateProfitLossDataSummary(apiData.profitloss_summary.invested_value, apiData.profitloss_summary.sold_value, apiData.profitloss_summary.pnl_value);
